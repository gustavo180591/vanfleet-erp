<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white shadow-sm rounded-lg p-6">
            <h2 class="text-lg font-semibold mb-4">Editar Contrato #{{ $contract->id }}</h2>

            <form action="{{ route('app.rental-contracts.update', $contract) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        >
                            @foreach($vehicles as $vehicle)
                                <option value="{{ $vehicle->id }}" 
                                    ...
                                    {{ old('vehicle_id', $contract->vehicle_id) == $vehicle->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select
                            ...
                            class="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 text-sm"
                        >
                            <option value="daily" {{ old('rental_type', $contract->rental_type) == 'daily' ? 'selected' : '' }}>Diario</option>
                            <option value="weekly" {{ old('rental_type', $contract->rental_type) == 'weekly' ? 'selected' : '' }}>Semanal</option>
                            <option value="monthly" {{ old('rental_type', $contract->rental_type) == 'monthly' ? 'selected' : '' }}>Mensual</option>
                        </select>
                        @error('rental_type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex rounded-md shadow-sm">
                            ...
                            <input
                                ...
                                name="monthly_rate"
                                id="monthly_rate"
                                value="{{ old('monthly_rate', $contract->monthly_rate) }}"
                                step="0.01"
                                min="0"
                                required
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea
                            ...
                            rows="4"
                            class="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 text-sm"
                        >{{ old('terms', $contract->terms) }}</textarea>
                        @error('terms')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
